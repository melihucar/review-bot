...
- **A pull request whose head moves no longer reports a failure.** Aborting a review because the head had moved — before the checkout, or at the re-read just before posting — was recorded like any other error: it spent one of the request's five retry attempts, wrote a "Failed" history entry carrying "attempt 1 of 5" wording, and left the menu bar in its red "Attention" state, since that state is read from the newest history entry. A release pull request that is pushed to often sat in "Attention" indefinitely while behaving exactly as designed. A moved head is now its own outcome: a "Head moved" history entry that says which commit replaced which, no attempt counted, and no red state. Nothing was ever at stake in counting it — the dedup key names a head that is already history, so the retry the budget paces is not the one that happens. The recorded count is also cleared rather than left behind, because that key can come back: a force-push to the same commit behind the same review request rebuilds it exactly, and stale failures would have started the fresh review part-way through its budget. A head re-read that *fails* is still a failure, since the head may well be unchanged.
- History no longer disappears when it contains an event kind the running build does not know. `history.json` is rewritten whole on every append and decoded all-or-nothing, so one entry written by a newer build — after reinstalling an older release, say — silently discarded all 2,000. Entries are now decoded one at a time, costing the unreadable entry instead of the file.
